...
function drawExactlyAdminModalHtml(productData, addNew) {
    ...
    managerModalHtml += "<div class=\"inputsData\">" +
        ...
        "<input class=\"inputData productName\" type=\"text\" required value=\" " + productData.name + "\">" +
        "<span>к-ть товару:</span>" +
        "<input class=\"inputData productAmount\" type=\"number\" required value=\" " + productData.amount + "\">" +
        "<span>строк придатності:</span>" +
        "<input type=\"text\"  id=\"datepicker\" readonly=readonly required>" +
        // '<input type="date" id="datepicker" data-date-inline-picker="true" onselect=getChosenDate min=' + dateString + '/>' +
        productData.expirationDate +
        "<span>ціна:</span>" +
        "<input class=\"inputData productPrice\" type=\"number\" min=1 required value=\" " + productData.price + "\">" +
        "<span>знижка:</span>" +
        "<input class=\"inputData productDiscount\" type=\"number\" min=0 required value=\" " + discount + "\">" +

        // ingredients
        "<span>Інгредієнти:</span>" +
        "<input type=text pattern=\"^[А-Яа-яЁё\s]+$\" class=\"productIngredients\" rows=\"4\" required>" + productData.stringIngredients + "</input>";
        // "<textarea class=\"productIngredients\" rows=\"4\" required>" + productData.stringIngredients + "</textarea>";
...
function validateForm() {
    var isValid = true;
    var $addProdModal = $(".addEditModal .modal-content .inputsData input")
    // var $addProdModal = $(".addEditModal .modal-content .inputsData .productIngredient")
    console.log($addProdModal)

    $addProdModal.each(function() {
      if ( $(this).val() === '' )
          isValid = false;
    });
    return isValid;
...
$(".addEditModal").click(function(event) {
    ...
    if ($(target).is(".save")) {
        ...
        validHtml = "<div class=fill-validation>Заповніть всі поля</div>"
        var validation = validateForm()
        if (validation != true) {
            $(".addEditModal .modal-content .inputsData").append(validHtml)
...
//         // var lettersCheck = new RegExp("/^[a-zA-Z]*$/");
//         var ingrLettersCheck = lettersCheck.test(ingredients);
    
//         if (!ingrLettersCheck || ingredients == "") {
//             uncorrectFields += "\n - інгредієнти";
//         }
//         return uncorrectFields;
//     }
